...
class Rucio(object):
    ...
    def listBlockNamesAndSizes(self, container, scope='cms'):
        """
        Given a container name, list all its blocks and block sizes
        :param containerName: string with the container DID
        :param scope: data scope, defaults to "cms"
        :return: a list of dictionary like:
            [{"name": "block_name", "bytes": 123}, etc]
        """
        results = []
        filters = dict(name=container + "#*", type="DATASET")
        resp = self.cli.list_dids(scope, filters=filters, long=True)
        for item in resp:
            results.append(dict(name=item['name'], bytes=item['bytes']))
        return results
    ...
    def pickRSE(self, rseExpression='rse_type=TAPE\cms_type=test', rseAttribute='ddm_quota', minNeeded=0):
        ...
        for rse in matching_rses:
            ...
            quota = attrs.get(rseAttribute, 0)
